...
public class SwipeViewIndicator extends LinearLayout {
    ...
    private OnSwipeViewIndicatorClickListener mOnPageControlClickListener = null;

    public SwipeViewIndicator(Context context) {
	super(context);
	mContext = context;
	initPageControl();
    }

    public SwipeViewIndicator(Context context, AttributeSet attrs) {
	super(context, attrs);
	mContext = context;
    }
	...
	setOnTouchListener(new OnTouchListener() {
		...
		if (mOnPageControlClickListener != null) {
			...
			if (SwipeViewIndicator.this.getOrientation() == LinearLayout.HORIZONTAL) {
			    if (event.getX() < (SwipeViewIndicator.this
				    .getWidth() / 2)) // if
						      // on
						      // left
						      // of
			    ...
			    if (event.getY() < (SwipeViewIndicator.this
				    .getHeight() / 2)) // if on top half of
						       // view
			    {
				if (mCurrentPage > 0) {
				    mOnPageControlClickListener.goBackwards();
	...
	for (int i = 0; i < pageCount; i++) {
	    ...
	    params.setMargins(mIndicatorSize / 2, mIndicatorSize,
		    mIndicatorSize / 2, mIndicatorSize);
     ...
     * 
     */
    public interface OnSwipeViewIndicatorClickListener {
	/**
     ...
     */
    public void setOnSwipeViewIndicatorClickListener(
	    OnSwipeViewIndicatorClickListener onPageControlClickListener) {
     ...
     *         PageControl
     */
    public OnSwipeViewIndicatorClickListener getOnPageControlClickListener() {
